...
def spark_submit(s3_bucket_script: str,input_script: str, event: dict)-> None:
    ...
    try:
        ...
        subprocess.run(["spark-submit", "--packages", "org.apache.spark:spark-hadoop-cloud_2.12:3.2.0", "/tmp/spark_script.py", "--event", json.dumps(event)], check=True, env=os.environ)
    except Exception as e :
